...
const createColorPickerMarkup = (images) =>
  images
    .map(
      (image) =>
        `<li class = "js-listimg" ><a href="${image.url}"><img src="${image.url}" alt="${image.alt}"></a></li>`
    )
    .join("");

const buttonsMarkup = createColorPickerMarkup(images);

gallery.insertAdjacentHTML("beforeend", buttonsMarkup);
